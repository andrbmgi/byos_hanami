...
Allows you to update an existing screen by rendering new HTML content. The screen's image will be replaced with the newly rendered content.
...
curl -X "PATCH" "https://localhost:2443/api/screens/1" \
  ...
  "screen": {
    "label": "Demo",
    "uri": "https://example.com/preprocessed-image.png",
    "preprocessed": true
  }
}'
----

Allows you to update an existing screen with a preprocessed image. The `preprocessed` value must be `true` when using a URI. This assumes the image at the URI is fully compatible with the device in terms of greyscale, bit depth, color depth, and so forth. We _do not process this image_ and only cache the image locally for display on your device.

All attributes except `uri` and `preprocessed` are optional. You can update metadata like `label`, `name`, or `model_id` along with the image in a single request.
====
